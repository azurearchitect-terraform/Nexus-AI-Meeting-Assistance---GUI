...
impl SpeakerInput {
    ...
    pub fn new() -> Result<Self> {
        let inner = PlatformSpeakerInput::new(None, None)?;
        Ok(Self { inner })
    }

    // Creates a new speaker input with a specific device ID
    #[cfg(any(target_os = "macos", target_os = "windows", target_os = "linux"))]
    pub fn new_with_device(device_id: Option<String>, is_input: Option<bool>) -> Result<Self> {
        let inner = PlatformSpeakerInput::new(device_id, is_input)?;
        Ok(Self { inner })
    }
    ...
    #[cfg(not(any(target_os = "macos", target_os = "windows", target_os = "linux")))]
    pub fn new_with_device(_device_id: Option<String>, _is_input: Option<bool>) -> Result<Self> {
        Err(anyhow::anyhow!(
