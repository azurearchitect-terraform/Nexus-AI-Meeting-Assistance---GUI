...
pub async fn start_system_audio_capture(
    ...
    let input = SpeakerInput::new_with_device(device_id, is_input).map_err(|e| {
...
// VAD-enabled capture - PERMANENT continuous loop that never exits
            ...
            if is_speech {
                ...
                if speech_buffer.len() > max_samples {
                    ...
                    if let Ok(b64) = samples_to_wav_b64(sr, &normalized_buffer) {
                        ...
                        // Notify JS that VAD is still running (no restart needed)
                        let _ = app.emit("vad-still-active", ());
                    }
                    speech_buffer.clear();
                    in_speech = false;
                    speech_chunks = 0;
                ...
                if in_speech {
                    ...
                    if silence_chunks >= config.silence_chunks {
                        ...
                        if speech_chunks >= config.min_speech_chunks && !speech_buffer.is_empty() {
                            ...
                            if let Ok(b64) = samples_to_wav_b64(sr, &normalized_buffer) {
                                let _ = app.emit("speech-detected", b64);
                                // Notify JS that VAD is STILL RUNNING continuously (no restart needed)
                                let _ = app.emit("vad-still-active", ());
                            } else {
                        ...
                        // Reset for next speech segment - loop CONTINUES RUNNING
                        speech_buffer.clear();
                        in_speech = false;
                        silence_chunks = 0;
                        speech_chunks = 0;
