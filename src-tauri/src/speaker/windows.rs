...
impl SpeakerInput {
    ...
    pub fn stream(self) -> SpeakerStream {
        ...
        let capture_thread = thread::spawn(move || {
            if let Err(e) =
                SpeakerStream::capture_audio_loop(queue_clone, waker_clone, init_tx, device_id, is_input)
            {
...
impl SpeakerStream {
    ...
    fn capture_audio_loop(
        ...
        let direction = if is_input.unwrap_or(false) { Direction::Capture } else { Direction::Render };

        'outer: loop {
            // Check shutdown before initializing
            {
            ...
            let init_result = (|| -> Result<_> {
                let device = match device_id {
                    Some(ref id) => match find_device_by_id(&direction, id) {
                        Some(d) => d,
                        None => get_default_device(&direction).expect("No default device"),
                    },
                ...
                Ok((h_event, render_client, sample_rate)) => {
                    ...
                    if !timeout_occurred {
                        if let Err(e) = render_client.read_from_device_to_deque(&mut temp_queue) {
                            error!("Pluely Failed to read audio data: {}. Attempting stream recovery...", e);
                            needs_reinit = true;
                        }
                    }
                    
                    if needs_reinit {
                        // Break inner loop to trigger re-initialization (e.g. bluetooth connected/disconnected)
                        std::thread::sleep(std::time::Duration::from_millis(500));
                        break;
                    }

                    if temp_queue.is_empty() && !timeout_occurred {
                        continue;
                    }

                    let mut samples = Vec::new();
                    if timeout_occurred {
                        // Inject 100ms of silence to keep VAD stream moving
                        let silence_samples = (sample_rate as f32 * 0.1) as usize;
                        samples.resize(silence_samples, 0.0f32);
                    } else {
            ...
            Err(e) => {
                if is_first_init {
                    ...
                    error!("Pluely stream re-initialization failed: {}. Retrying...", e);
                    std::thread::sleep(std::time::Duration::from_millis(1000));
                    continue 'outer;
                }
            }
        } // End of match init_result
        } // End of 'outer loop
