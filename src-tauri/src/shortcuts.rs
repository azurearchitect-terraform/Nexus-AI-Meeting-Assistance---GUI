...
pub fn handle_shortcut_action<R: Runtime>(app: &AppHandle<R>, action_id: &str) {
    match action_id {
        ...
        "panic_button" => handle_panic_button(app),
        custom_action => {
...
/// Handle Panic Button (Instantly hide window and clear data)
fn handle_panic_button<R: Runtime>(app: &AppHandle<R>) {
    let Some(window) = app.get_webview_window("main") else {
        return;
    };

    // Simply exit the application immediately
    app.exit(0);
}
