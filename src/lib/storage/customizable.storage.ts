...
export const getCustomizableState = (): CustomizableState => {
  try {
    ...
    return {
      ...
      privacyMode:
        parsedState.privacyMode || DEFAULT_CUSTOMIZABLE_STATE.privacyMode,
      offlineMode:
        parsedState.offlineMode || DEFAULT_CUSTOMIZABLE_STATE.offlineMode,
    };
  } catch (error) {
...
/**
 * Update Privacy Mode state
 */
export const updatePrivacyMode = (isEnabled: boolean): CustomizableState => {
  const currentState = getCustomizableState();
  const newState = { ...currentState, privacyMode: { isEnabled } };
  setCustomizableState(newState);
  return newState;
};
 ...
 */
export const updateOfflineMode = (isEnabled: boolean): CustomizableState => {
  const currentState = getCustomizableState();
  const newState = { ...currentState, offlineMode: { isEnabled } };
  setCustomizableState(newState);
  return newState;
};
