...
export const CUSTOM_PROMPTS_KEY = "nexus_custom_prompts";

export const getCustomPrompts = (): PromptTemplate[] => {
  try {
...
export const saveCustomPrompts = (prompts: PromptTemplate[]) => {
...
export const getAllPrompts = (): PromptTemplate[] => {
  return [...PROMPT_TEMPLATES, ...getCustomPrompts()];
};
...
Provide insights, key points, and actionable information as the meeting progresses. For complex technical or process-related topics, provide comprehensive, structured explanations.`,
  },
  {
...
Listen to interview questions and provide relevant talking points. For behavioral questions, keep it concise. For technical, architectural, or process questions (e.g. cloud migrations), provide detailed, structured, and comprehensive explanations to help the candidate answer effectively.`,
  },
  {
...
Listen to technical questions and provide helpful guidance. If the question is simple, be concise. If the question involves a complex process (like cloud migration, system design, or architecture), provide a highly detailed, step-by-step, and comprehensive explanation.`,
  },
  {
...
export const getPromptTemplateById = (id: string): PromptTemplate | undefined => {
  return getAllPrompts().find((template) => template.id === id);
};

export const getPromptTemplateNames = (): { id: string; name: string }[] => {
  return getAllPrompts().map((template) => ({
    id: template.id,
    name: template.name,
  }));
};
