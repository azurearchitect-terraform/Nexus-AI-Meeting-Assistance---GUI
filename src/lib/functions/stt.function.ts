...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    for (const [key, val] of Object.entries(selectedProvider.variables || {})) {
      ...
      // Aggressively sanitize Gemini model names for STT, as curl urls expect raw model name without models/
      if (uppercaseKey === "MODEL" && typeof valueToStore === "string") {
         valueToStore = valueToStore.replace(/^models\//, "");
      }
      varsObj[uppercaseKey] = valueToStore;
    }
    ...
    // Ensure MODEL is set for Gemini STT if missing
    if (provider.id === "gemini-stt" || provider.id === "gemini") {
        if (!allVariables.MODEL) {
            allVariables.MODEL = provider?.defaultModel || "gemini-3.5-flash";
        }
    }
    // Prepare request
