...
export const PluelyApiSetup = () => {
  ...
  const title = isModelsLoading
    ? "Loading Models..."
    : `Nexus supports ${models?.length} model${
        models?.length !== 1 ? "s" : ""
      }`;
  ...
  const description = isModelsLoading
    ...
    : "Explore all the models Nexus supports.";
  ...
  return (
    <div id="pluely-api" className="space-y-3 -mt-2">
      <div className="space-y-2 pt-2">
        ...
        {/* License Key Input Removed */}
      </div>
      <div className="flex justify-between items-center">
        <Header
          title={`${pluelyApiEnabled ? "Disable" : "Enable"} Nexus API`}
          description={
            pluelyApiEnabled
              ? "Using Nexus APIs for audio and chat."
              : "Using your own custom AI Providers for audio and chat."
          }
        />
        <Switch
