...
// Create the context
export const AppContext = createContext<IContextType | undefined>(undefined);
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [companyUrl, setCompanyUrl] = useState<string>(
    safeLocalStorage.getItem(STORAGE_KEYS.COMPANY_URL) || ""
  );
  ...
  const loadData = () => {
    ...
    // Load company URL
    const savedCompanyUrl = safeLocalStorage.getItem(
      STORAGE_KEYS.COMPANY_URL
    );
    if (savedCompanyUrl !== null) {
      setCompanyUrl(savedCompanyUrl);
    }
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      ...
      if (
        ...
        e.key === STORAGE_KEYS.COMPANY_URL ||
